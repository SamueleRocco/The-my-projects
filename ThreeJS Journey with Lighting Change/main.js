...
Tree = function () {
	...
	var matTreeLeaves = new THREE.MeshPhongMaterial( { color:Colors.green, shading:THREE.SmoothShading});
...
function loop(){
  ...
  var verticalMovement = amplitude * Math.sin(frequency * Date.now());

  // Move the sun up and down
  sun.mesh.position.y = -30 - verticalMovement;
  var yPosition = sun.mesh.position.y;

  // Calculate the ambient light intensity based on the sun's vertical position
  var ambientIntensity =  Math.min(Math.max(sun.mesh.position.y,.3),.7) ;
  hemisphereLight.intensity = ambientIntensity;
  
   var backgroundColor = 'rgb(' + Math.round(Math.max(yPosition/3,70)) + ', ' + Math.round(Math.max(yPosition/3,70)) + ', ' + Math.round(Math.max(yPosition,150)) + ')';
  container.style.backgroundColor = backgroundColor;

  // Set the opacity of the canvas (invert the opacity value since we want it more opaque when the sun is up)

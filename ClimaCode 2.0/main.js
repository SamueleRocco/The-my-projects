...
toggles.forEach(function(toggle) {
  toggle.addEventListener('click', function() {
    ...
    if (this.id === 'toggle-cel') {
      ...
      tempElement.textContent = celsius + '°F';
    } else if (this.id === 'toggle-far') {
      var fahrenheit = Math.round(tempValue * 9 / 5 + 32);
      tempElement.textContent = fahrenheit + '°c';
    }
  });
});
...
function changeTemp(element, newTemp) {
  ...
  let currentTemp = unit === "°C" ? currentTempF : Math.round((currentTempF - 32) * 5 / 9);
  let finalTemp = unit === "°C" ? newTemp : Math.round((newTemp - 32) * 5 / 9);
